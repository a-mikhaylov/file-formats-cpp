...
int prqt_test::Test2_read(Log& test_Log, std::vector<int> quants, std::vector<arrow::Compression::type> compressions)
{
    FileRunInfo info;

    std::string cur_path(boost::filesystem::current_path().c_str());
    std::cerr << "[TEST]: prqt_test::Test2_read() - STARTED: "
        ...
        for (int QUANT : quants) {
            for (arrow::Compression::type compr : compressions) { 
...

                {
                    ...
                    BinWriter BWriter{
                        data_dir + prqt_settings::GenerateBinName(file_title, QUANT, compr)
                        };
                    bool need_go = true;
                    while(true) {
